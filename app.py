...
def find_similar_movies(movie_id, ratings_df, movies_df):
    ...
    similar_users = ratings_df.query(f"movieId == {movie_id} & rating > 4")["userId"].unique()
    similar_user_recs = (
        ratings_df.query(f"userId.isin({list(similar_users)}) & rating > 4")["movieId"]
        .value_counts() / len(similar_users)
    )
    similar_user_recs = similar_user_recs[similar_user_recs > 0.10]
    
    all_users = (
        ratings_df.query(f"movieId.isin({list(similar_user_recs.index)}) & rating > 4")
    )
